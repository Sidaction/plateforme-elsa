...
$poi[] = array('lat'=> $lat, 'lng'=> $lng, 'slug'=>$slug, 'title'=>get_the_title());
 ?>
        ...
        <div class="antennes">
			...
			$wp_query = new WP_Query($args);
				if ($wp_query->have_posts()) while ($wp_query->have_posts()) : $wp_query->the_post();
                        $coord=get_post_meta($post->ID, 'loc', true);
						list( $lat, $lng ) = explode( ',', $coord);
						if(!empty($coord)) $poi[] = array('lat'=> $lat, 'lng'=> $lng, 'slug'=>$slug);
						endwhile; wp_reset_query();wp_reset_postdata(); $args=null; ?>
			 <?php if(!empty($poi)):?>
             	<div id="mapPays" style="width:260px;height:300px"></div>
