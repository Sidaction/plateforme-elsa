...
<section id="contentSite" class="marron home">
	<div id="breadcrumb">
     ...
     <div id="sliderHome">
      <?php 
	  		$i=0;
	  		$args = array('post_type' => array('diaporama'), 'posts_per_page' => 4);
			$wp_query = new WP_Query($args);
				if ($wp_query->have_posts()) while ($wp_query->have_posts()) : $wp_query->the_post();?>
     <?php if($i==0) :?>
    	<div class="infosSlide">
        <div class="infostitreSlide"><?php echo get_post_meta($post->ID, 'surtitre', true);?></div>
        <hr />
        <strong><?php the_title();?></strong>
        <p><?php the_excerpt();?></p>
        <a href="<?php echo get_post_meta($post->ID, 'link', true);?>" <?php if(get_post_meta($post->ID, 'blank', true)=='1') echo 'target="_blank"';?>>» en savoir plus</a>
        </div>
        <div class="imgSlide">
        ...
        <div class="listSlide">
			...
			<?php else :?>
            	<li>
                <a href="<?php echo get_post_meta($post->ID, 'link', true);?>" <?php if(get_post_meta($post->ID, 'blank', true)=='1') echo 'target="_blank"';?>>	
               	  <div class="thumbListslide"><?php the_post_thumbnail('thumbnail');?></div>
                    <div class="txtListslide">
                        <div class="titleSlide"><?php echo get_post_meta($post->ID, 'surtitre', true);?></div>
                    <p><?php the_title();?></p>
                    </div>
